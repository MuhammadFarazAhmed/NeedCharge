...
class RoundedCornersTransformation @JvmOverloads constructor(
    ...
    enum class CornerType {
        ...
        DIAGONAL_FROM_TOP_LEFT, DIAGONAL_FROM_TOP_RIGHT, DIFFERENT
    }
    ...
    private fun drawRoundRect(canvas: Canvas, paint: Paint, width: Float, height: Float) {
        ...
        when (cornerType) {
            ...
            CornerType.DIFFERENT -> {
                val rect = Rect(0, 0, width.toInt(), height.toInt())
                val rectF = RectF(rect)
                drawAsymmetricRoundRect(canvas, rectF, floatArrayOf(16f, 16f, 34f, 34f), paint)
            }
        }
    }
    ...
    private fun drawTopLeftRoundRect(canvas: Canvas, paint: Paint, right: Float, bottom: Float) {
        canvas.drawRoundRect(
                RectF(margin.toFloat(), margin.toFloat(), (margin + diameter).toFloat(), (margin + diameter).toFloat()),
                radius.toFloat(),
                radius.toFloat(),
                paint
        )
    ...
    private fun drawBottomLeftRoundRect(canvas: Canvas, paint: Paint, right: Float, bottom: Float) {
        canvas.drawRoundRect(
                RectF(margin.toFloat(), bottom - diameter, (margin + diameter).toFloat(), bottom), radius.toFloat(),
                radius.toFloat(), paint
        )
        canvas.drawRect(
    ...
    private fun drawAsymmetricRoundRect(canvas: Canvas, rectF: RectF, radii: FloatArray, paint: Paint) {
        val topLeftX = rectF.left + radii[0]
        val topLeftY = rectF.top + radii[0]
        val topRightX = rectF.right - radii[1]
        val topRightY = rectF.top + radii[1]
        val bottomRightX = rectF.right - radii[2]
        val bottomRightY = rectF.bottom - radii[2]
        val bottomLeftY = rectF.bottom - radii[3]
        val bottomLeftX = rectF.left + radii[3]
        val topLeftCorner = RectF(rectF.left, rectF.top, topLeftX + radii[0], topLeftY + radii[0])
        val topRightCorner = RectF(topRightX - radii[1], rectF.top, rectF.right, topRightY + radii[1])
        val bottomRightCorner = RectF(bottomRightX - radii[2], bottomRightY - radii[2], rectF.right, rectF.bottom)
        val bottomLeftCorner = RectF(rectF.left, bottomLeftY - radii[3], bottomLeftX + radii[3], rectF.bottom)

        canvas.drawArc(topLeftCorner, 180f, 90f, true, paint)
        canvas.drawArc(topRightCorner, 270f, 90f, true, paint)
        canvas.drawArc(bottomRightCorner, 0f, 90f, true, paint)
        canvas.drawArc(bottomLeftCorner, 90f, 90f, true, paint)
        canvas.drawRect(topLeftX, rectF.top, topRightX, if (bottomLeftY < bottomRightY) bottomLeftY else bottomRightY, paint) //top rect
        canvas.drawRect(if (topLeftX > bottomLeftX) topLeftX else bottomLeftX, topRightY, rectF.right, bottomRightY, paint) //right rect
        canvas.drawRect(bottomLeftX, if (topLeftY > topRightY) topLeftY else topRightY, bottomRightX, rectF.bottom, paint) //bottom rect
        canvas.drawRect(rectF.left, topLeftY, if (bottomRightX < topRightX) bottomRightX else topRightX, bottomLeftY, paint) //left rect
    }
    ...
    companion object {
        ...
        private val ID = "needCharge$VERSION"
    }
}
